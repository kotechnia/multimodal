...
def multichoice_encoding_data(args, vocab, questions, video_names, video_ids, answers, ans_candidates, tokenizer,  mode='train'):
...
def process_questions_mulchoices(args):
    ...
    if args.mode in ["train", "val"]:
        csv_data = pd.read_csv(args.annotation_file.format("Train", args.question_type), delimiter='\t',encoding='utf-8-sig')
    else:
        csv_data = pd.read_csv(args.annotation_file.format("Test", args.question_type), delimiter='\t', encoding='utf-8-sig')

    if args.token_type == "transformers":
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer if args.tokenizer else 'monologg/koelectra-base-v3-discriminator')
    else: 
        tokenizer = nltk
    ...
    if args.mode in ['train']:
        ...
        for candidates in ans_candidates:
            for ans in candidates:
                ...
                for token in tokenizer.tokenize(ans):
        ...
        if args.by_video == 'n':
            ...
            multichoice_encoding_data(args, vocab, train_questions, train_video_names, train_video_ids, train_answers, train_ans_candidates, tokenizer=tokenizer, mode='train')
            ...
            for i, mode in enumerate(['train', 'validation', 'test']):
               ...
               idx = [i for i, _ in enumerate(video_ids) if _ in split_list[mode] ] 

               mode_questions = list(np.array(questions)[idx])
               mode_answers = list(np.array(answers)[idx])
               mode_video_names = list(np.array(video_names)[idx])
               mode_video_ids = list(np.array(video_ids)[idx])
               mode_ans_candidates = list(np.array(ans_candidates)[idx, :])
               if mode =='validation':
                   mode = 'val'
               multichoice_encoding_data(args, vocab, mode_questions, mode_video_names, mode_video_ids, mode_answers, mode_ans_candidates, tokenizer=tokenizer, mode=mode)
        ...
        multichoice_encoding_data(args, vocab, questions, video_names, video_ids, answers,
                                  ans_candidates, tokenizer=tokenizer, mode='test')
